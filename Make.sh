...
function debug() {
    ...
    dotnet build "$BASE_DIRECTORY/src/Medo.Rabbit.sln" \
                 --framework "net7.0" \
                 --configuration "Debug" \
                 --output "$BASE_DIRECTORY/build/debug/" \
                 --verbosity "minimal" \
                 || return 1
    cp "$BASE_DIRECTORY/build/debug/Medo.Rabbit.dll" "$BASE_DIRECTORY/bin/" || return 1
    cp "$BASE_DIRECTORY/build/debug/Medo.Rabbit.pdb" "$BASE_DIRECTORY/bin/" || return 1
...
function release() {
    ...
    dotnet build "$BASE_DIRECTORY/src/Medo.Rabbit.sln" \
                 ...
                 --verbosity "minimal" \
                 || return 1
    cp "$BASE_DIRECTORY/build/release/Medo.Rabbit.dll" "$BASE_DIRECTORY/bin/" || return 1
    cp "$BASE_DIRECTORY/build/release/Medo.Rabbit.pdb" "$BASE_DIRECTORY/bin/" || return 1
    echo "${ANSI_CYAN}Output in 'bin/'${ANSI_RESET}"
}
...
function package() {
    ...
    dotnet pack "$BASE_DIRECTORY/src/Medo.Rabbit.csproj" \
                --configuration "Release" \
                --force \
                --include-source \
...
function test() {
    ...
    echo ".NET `dotnet --version`"
    dotnet test "$BASE_DIRECTORY/src/Medo.Rabbit.sln" \
                --configuration "Debug" \
...
PACKAGE_ID=`cat "$BASE_DIRECTORY/src/Medo.Rabbit.csproj" | grep "<PackageId>" | sed 's^</\?PackageId>^^g' | xargs`
PACKAGE_VERSION=`cat "$BASE_DIRECTORY/src/Medo.Rabbit.csproj" | grep "<Version>" | sed 's^</\?Version>^^g' | xargs`
